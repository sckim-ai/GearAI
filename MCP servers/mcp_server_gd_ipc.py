...
def show_snapshot(image_path: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "html_tag": f'<img src="data:image/{image_format};base64,{image_base64}" alt="Gear Design Image" style="max-width: 100%; height: auto;">'
        }
...
@mcp.tool()
def get_latest_image(session_id: str) -> dict:
    """
    세션의 가장 최근 이미지를 가져옵니다.
    ...
    try:
        ...
        # 이미지 파일만 필터링
        image_files = [f for f in session.files if f.get("type") in ["image", "3d_image"]]
