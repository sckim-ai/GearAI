    ...
    if current_shaft == {pinion_shaft_name}:
        ...
        try:
            ...
            if hasattr(current_conn.wrapped, 'AxialPosition'):
                ...
                print(f"[위치 변경] 피니언: {pinion_shaft_name}에서 {pinion_position}mm로 위치 변경")
            else:
                ...
                print(f"[재장착 완료] 피니언: {pinion_shaft_name}에 {pinion_position}mm 위치에 재장착")
        except Exception as e:
    ...
    if current_shaft == {wheel_shaft_name}:
        ...
        try:
            if hasattr(current_conn.wrapped, 'AxialPosition'):
                ...
                print(f"[위치 변경] 휠: {wheel_shaft_name}에서 {wheel_position}mm로 위치 변경")
            else:
                # AxialPosition이 없으면 재장착
                print(f"[재장착] 휠: 위치 변경 불가, 재장착 수행")
                {wheel_shaft_name}.mount_component(wheel_mount, {wheel_position}*MM)
                print(f"[재장착 완료] 휠: {wheel_shaft_name}에 {wheel_position}mm 위치에 재장착")
        except Exception as e:
